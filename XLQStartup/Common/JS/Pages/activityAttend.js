...
$(function () {
    ...
    share(document.title, "我发现了一个精彩的活动，快来一起参与吧！", "http://wechat.adobe-event.cn/pages/Activity/attend.html?id=" + args.id);
});
    ...
    easyweb.server.load(apiUrl, null,
        function (result) {
            ...
            if (member == null || (member.RegTime == null || member.RegTime == "" || member.RegTime == "0001-01-01 00:00:00")) {
                window.location = "/Pages/Member/register.html?openId=" + openId + "&actId=" + args.id + "&from=attend";
            }
            else {
                var id = activity.Id;
                var apiUrl1 = "/API/Activity/AttendActivity";
                easyweb.server.load(apiUrl1, { "activityId": id, "openId": openId },
                    function (result) {
                        //将按钮处理
                        $("#attendBtn").hide();
                        $("#detailBtn").show();
